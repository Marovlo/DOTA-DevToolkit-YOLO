...
class splitbase():
    def __init__(self,
        ...
        """
        :param basepath: base path for dota data  待分割的文件夹
        :param outpath: output base path for dota data,  输出分割图片的文件夹
        the basepath and outputpath have the similar subdirectory, 'images' and 'labelTxt'
        :param code: encodeing format of txt file
        :param gap: overlap between two patches  两张图片之间的重叠
...
if __name__ == '__main__':
    ...
    split_size=300
    gap=50
    if os.path.exists(rf'/data/hatim/zzl/1dota/{split_size}trainsplit'):
        shutil.rmtree(rf'/data/hatim/zzl/1dota/{split_size}trainsplit')
        shutil.rmtree(rf'/data/hatim/zzl/1dota/{split_size}valsplit')
    os.mkdir(rf'/data/hatim/zzl/1dota/{split_size}trainsplit')
    os.mkdir(rf'/data/hatim/zzl/1dota/{split_size}valsplit')
    splittrain = splitbase(r'/data/hatim/zzl/1dota/train',
                      rf'/data/hatim/zzl/1dota/{split_size}trainsplit',
                      gap=gap,  # 重叠区域
                      subsize=split_size,  # 被分割成的小图片的size
                      num_process=16
                      )
    splittrain.splitdata(1)

    print('split val')
    splitval = splitbase(r'/data/hatim/zzl/1dota/val',
                      rf'/data/hatim/zzl/1dota/{split_size}valsplit',
                      gap=gap,        # 重叠区域
                      subsize=split_size,   # 被分割成的小图片的size
                      num_process=16
                      )
    splitval.splitdata(1)  # resize rate before cut
